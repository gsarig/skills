...
The key insight that `description` should contain only triggering conditions and never summarize the skill's workflow (because agents shortcut-follow the summary instead of reading the full skill) comes from Jesse Vincent's [superpowers](https://github.com/obra/superpowers) framework, discovered through [this review](https://ai-resources.gsarigiannidis.gr/reviews/superpowers/). The rest of the structure, field taxonomy, and pressure-testing approach were developed independently for Claude Code's SKILL.md format. More context on the workflow that produced that review is in [this blog post](https://www.gsarigiannidis.gr/claude-code-obsidian-ai/).
